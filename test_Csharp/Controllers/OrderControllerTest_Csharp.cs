...
namespace Microsoft.eShopWeb.Web.Tests.Controllers
{
    public class OrderControllerTests
    {
        ...
        {
            // Arrange
            var otherUserOrder = new Order("otheruser@example.com", new Address("1", "2", "3", "4", "5"));
            var customerOrders = new List<Order> { otherUserOrder };
            _mockOrderRepository.Setup(x => x.ListAsync(It.IsAny<CustomerOrdersWithItemsSpecification>()))
                .ReturnsAsync(customerOrders);

            // Act
            var result = await _controller.Detail(otherUserOrder.Id);
        ...
        private List<Order> GetTestOrders()
        {
            ...
            {
                new Order(TestUserName, address)
                {
                    Id = 1,
